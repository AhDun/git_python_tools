...
def git_log():
    ...
    sum = 0
    if shell_is_error() == 1:
         messagebox.showerror("错误", "查询历史提交记录错误")
    else:
        history = ''
        outputs = shell_output
        while True:
...
def git_clone():
    ...
    url = simpledialog.askstring(title='',prompt='输入仓库地址')
    if url  == None:
        return
    dirname = url[url.rfind('/'):url.rfind('.git')]
    if sys.platform == "win32":
        dirname = dirname.replace('/','\\')
    if len(dirname) == 0:
        messagebox.showerror("错误", "仓库地址无效")
        return
...
def git_pull():
    ...
    if shell_output.find("Changes not staged for commit:") != -1:
        messagebox.showerror("严重警告", "严重警告：请在拉取前先本地提交,否则本地文件将会被覆盖掉！！！")
        shell_win_insert_separator('仓库拉取已取消')
        return
    if shell_output.find("Untracked files:") != -1:
        messagebox.showerror("严重警告", "严重警告：请在拉取前先本地提交,否则本地文件将会被覆盖掉！！！")
        shell_win_insert_separator('仓库拉取已取消')
        return
    if shell_output.find("Changes to be committed:") != -1:
        messagebox.showerror("严重警告", "严重警告：请在拉取前先本地提交,否则本地文件将会被覆盖掉！！！")
        shell_win_insert_separator('仓库拉取已取消')
        return
    if default_branch_dialog == True:
...
def about():
    ...
    w_label = Label(win, text="作者: AhDun", font=("", 12))
    w_label.pack()
...
main_menu.add_command (label="提交(F1)", command=git_add_commit)
root.bind("<F1>",lambda event:git_add_commit())
main_menu.add_command (label="拉取(F3)", command=git_pull)
root.bind("<F3>",lambda event:git_pull())
main_menu.add_command (label="推送(F5)", command=git_push)
root.bind("<F5>",lambda event:git_push())
main_menu.add_command (label=" | ")

main_menu.add_command (label="克隆(F7)", command=git_clone)
root.bind("<F7>",lambda event:git_clone())
main_menu.add_command (label="新建(F9)", command=git_create)
root.bind("<F9>",lambda event:git_create())
...
#push_menu.add_command (label="本地提交并仓库推送",command=git_add_commit_push)
push_menu.add_command (label="本地提交",command=git_add_commit)
push_menu.add_command (label="仓库推送",command=git_push)

push_menu.add_command (label=" ")

push_menu.add_command (label="撤回本地提交",command=ungit_add_commit)
